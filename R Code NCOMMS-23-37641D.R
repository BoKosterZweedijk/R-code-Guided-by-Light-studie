...
#---------------------------Difference tumor and background MFI-------------------------
...
FigMFIgewogen <- ggplot(MFI, aes(x = dose, y = `mfi gewogen`, fill = type)) +
  ...
  # **Boxplot kleuren aanpassen**
  scale_fill_manual(values = c(
    "Tumor" = "aliceblue",
    "Background" = "deepskyblue3"
  )) +
  # Significatielijnen via stat_pvalue_manual
  stat_pvalue_manual(
    pvals,
    label = "label",
    xmin = "xmin",
...
# Opslaan naar Desktop
ggsave(
  filename = "~/Desktop/MDSFR SFF plot.png",
  plot = p,
  width = 10,
  height = 6,
  dpi = 300
...
#----Difference in number of inadequate margins------
